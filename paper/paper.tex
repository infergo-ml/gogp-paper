...
on inference outcomes. To choose the best one for each particular
application, probabilistic programming languages and systems are
customarily compared by the expressive power (classes of
probabilistic models that can be represented), conciseness (e.g.
the number of lines of code to describe a particular model), and
time and space efficiency of inference
algorithms~\cite{WVM14,P16,R17,GXG18}.  However, complex
software systems make integration of probabilistic inference
challenging, and considerations between beyond expressiveness
...
A flexible data interface is also benefitial for support of
...
\cite{R16} \cite{KB15} \cite{N12} \cite{HG11} \cite{LN89}
