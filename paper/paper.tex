...
before inference. However, this is by no means required by
...
schools (Section~\ref{sec:8-schools}) is a popular example  we
use to compare model specification in Stan and Infergo. Linear
regression (Session~\ref{sec:8-schools}) formulated as a
probabilistic program lets us explore a model comprising
multiple methods and illustrate the inference-simulation use
case. Gaussian mixture (Section~\ref{sec:gaussian-mixture}) is
the most complex model of the three, featuring nested loops and
dynamic control structure.

By no means the models presented in this section reflect 
...
The eight schools problem~\cite{GCS+03} 
